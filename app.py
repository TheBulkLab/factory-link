...
COMPLEX_DB = {
    "서울": {
        "서울디지털(구로/가산)": [37.4800, 126.8850],
        "마곡일반": [37.5600, 126.8300],
        "온수일반": [37.4900, 126.8200]
    },
    "경기/인천": {
        "시화국가": [37.3275, 126.7350], 
        "반월국가": [37.3140, 126.7900], 
        "남동국가": [37.4050, 126.6900], 
        "평택(포승/고덕)": [36.9350, 126.8500], 
        "화성(향남/동탄)": [37.1300, 126.9000], 
        "파주(LCD/출판)": [37.7600, 126.7800], 
        "김포골드밸리": [37.6200, 126.6000],
        "인천일반(주안/부평)": [37.5000, 126.6700],
        "용인반도체": [37.2200, 127.1900]
    },
    "충청권": {
        "대산석유화학": [36.9900, 126.4200], 
        "당진철강": [36.9500, 126.7500], 
        "오창과학": [36.7100, 127.4300], 
        "청주일반": [36.6400, 127.4300], 
        "대전대덕테크": [36.4300, 127.4000], 
        "서산테크노": [36.8500, 126.5000],
        "세종첨단": [36.5500, 127.2800],
        "아산디스플레이": [36.8000, 127.0700]
    },
    "경상권": {
        "울산미포": [35.5000, 129.3500], 
        "온산국가": [35.4300, 129.3300], 
        "포항철강": [35.9900, 129.3700], 
        "구미국가": [36.1100, 128.3600], 
        "창원국가": [35.2100, 128.6600], 
        "대구성서": [35.8400, 128.5000], 
        "부산녹산": [35.0900, 128.8700],
        "거제옥포": [34.8800, 128.7000],
        "양산일반": [35.3300, 129.0200]
    },
    "전라권": {
        "여수국가(산단)": [34.8200, 127.7000], 
        "광양제철": [34.9300, 127.7300], 
        "군산국가": [35.9500, 126.5500], 
        "광주첨단": [35.2200, 126.8500], 
        "대불국가": [34.7800, 126.4500],
        "익산국가": [35.9700, 127.0000],
        "전주탄소": [35.8200, 127.1000]
    },
    "강원/기타": {
        "원주문막": [37.3300, 127.8500],
        "춘천후평": [37.8800, 127.7500],
        "강릉과학": [37.7500, 128.8700],
        "제주첨단": [33.4500, 126.5500]
    }
}
...
def login_page():
    ...
    with col1:
        ...
        st.markdown("<div class='main-header'>Factory Link <span style='font-size:1.5rem; color:#64748B;'>V1.9</span></div>", unsafe_allow_html=True)
...
def main_app():
    ...
    with tabs[0]:
        ...
        with st.container(border=True):
            ...
            c1, c2 = st.columns(2)
            with c1: 
                all_regions = list(COMPLEX_DB.keys())
                f_region = st.multiselect("📍 권역", all_regions)
