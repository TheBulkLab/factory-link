...
# === 데이터 구조 ===
COLS_RESOURCES = ["id", "writer_id", "date", "company", "contact", "region", "role", "category", "item", "lat", "lon", "desc", "process", "verified", "image_path"]
...
# === [심플화] 권역별 중심 좌표 DB ===
# 산단을 선택하지 않고, 권역만 선택하면 해당 지역 중심으로 랜덤 좌표 생성
REGION_DB = {
...
def login_page():
    ...
    with col1:
        ...
        st.markdown("<div class='main-header'>Factory Link <span style='font-size:1.5rem; color:#64748B;'>V1.5</span></div>", unsafe_allow_html=True)
        ...
        with st.container(border=True):
            ...
            with t2:
                ...
                with st.form("signup_form"):
                    ...
                    comp_name = st.text_input("기업명 (선택)")
                    ...
                    if signup_submitted:
                        try:
                            ...
                            elif len(users) > 0 and clean_new_id in users['user_id'].values:
                                st.error("중복된 아이디입니다.")
                            else:
                                new_user = {
...
def main_app():
    ...
    st.markdown("<div class='main-header'>🏭 Factory Link <span style='font-size:1.5rem; color:#64748B;'>V1.9.1</span></div>", unsafe_allow_html=True)
    ...
    with tabs[0]:
        ...
        with st.container(border=True):
            ...
            with c_filter:
                f_role = st.multiselect("거래 구분", ["팝니다", "삽니다", "수거/운송", "분석/기타"])

            c1, c2 = st.columns(2)
            with c1: 
                f_region = st.multiselect("📍 지역 선택", list(REGION_DB.keys()))
            with c2: 
                f_cat = st.multiselect("📦 카테고리", list(CATEGORIES))
        ...
        if len(df) > 0:
            ...
            if f_region: filtered_df = filtered_df[filtered_df['region'].isin(f_region)]
            ...
            for idx, row in filtered_df.iterrows():
                with st.container(border=True):
                    ...
                    with c1:
                        ...
                        st.caption(f"📍 {row['region']} | 📂 {row['category']} | {row['role']}")
                        st.write(row['desc'])
                    with c2:
                        if row['writer_id'] == st.session_state['user_id']:
                            st.button("내 글", disabled=True, key=f"mine_{idx}")
                        else:
                            my_req = msgs[(msgs['from_user'] == st.session_state['user_id']) & (msgs['item_id'] == str(row['id']))] if len(msgs) > 0 else pd.DataFrame()
                            if len(my_req) == 0:
    ...
    with tabs[1]:
        ...
        # [수정] st.form 사용하지 않음 (반응성 향상 및 로딩 최소화)
        c1, c2 = st.columns(2)
        with c1:
            region = st.selectbox("권역 (필수)", list(REGION_DB.keys()))
        with c2:
            role = st.selectbox("거래 구분", ["팝니다", "삽니다", "수거/운송", "분석/기타"])
        ...
        st.divider()
        company = st.text_input("기업명", value=curr_user['company_name'])
        contact = st.text_input("담당자 연락처 (승인된 회원에게만 공개)", value=curr_user['contact'])

        if st.button("등록 완료", type="primary", use_container_width=True):
            if not item_name or not desc:
                st.error("제목과 상세 내용은 필수입니다.")
            else:
                # [수정] 랜덤 좌표 생성 (산단 DB 의존 X)
                base_coords = REGION_DB[region]
                lat = base_coords[0] + random.uniform(-0.1, 0.1) # 10km 반경 랜덤
                ...
                new_data = {
                    ...
                    "company": company, "contact": contact, "region": region, "role": role,
                    "category": cat_main, "item": item_name, "lat": lat, "lon": lon,
    ...
    with tabs[2]:
        ...
        with t_in:
            if len(msgs) > 0:
                ...
                if len(my_recv) == 0: st.info("받은 요청이 없습니다.")
                for idx, row in my_recv.iterrows():
                    # [수정] 에러 방지용 조건문 추가
                    req_user_df = users[users['user_id'] == row['from_user']]
                    if len(req_user_df) > 0:
                        req_user = req_user_df.iloc[0]
                        res_item_df = resources[resources['id'] == str(row['item_id'])]
                        title = res_item_df.iloc[0]['item'] if len(res_item_df) > 0 else "(삭제됨)"
        ...
        with t_out:
            if len(msgs) > 0:
                ...
                if len(my_sent) == 0: st.info("보낸 요청이 없습니다.")
                for idx, row in my_sent.iterrows():
                    res_item_df = resources[resources['id'] == str(row['item_id'])]
                    if len(res_item_df) > 0:
                        target = res_item_df.iloc[0]
