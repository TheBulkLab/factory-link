                ...
                with st.form("login"):
                    ...
                    if st.form_submit_button("로그인", use_container_width=True):
                        try:
                            ...
                            # === [핵심 수정] 모바일 오타 방지 (공백 제거) ===
                            clean_uid = uid.strip()  # 아이디 앞뒤 공백 제거
                            clean_pw = upw.strip()   # 비번 앞뒤 공백 제거
                ...
                with st.form("signup"):
                    ...
                    if st.form_submit_button("가입신청", use_container_width=True):
                        try:
                            ...
                            if not users.empty and clean_new_id in users['user_id'].values:
                                st.error("중복된 아이디")
                            else:
                                new_user = {"user_id": clean_new_id, "password_hash": hash_password(clean_new_pw), "company_name": comp_name, "contact": contact, "biz_no": "-", "is_verified": "FALSE", "deal_count": 0, "reputation": 36.5, "join_date": datetime.now().strftime("%Y-%m-%d")}
