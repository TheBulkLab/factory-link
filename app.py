...
def login_page():
    ...
    with col1:
        ...
        st.markdown("<div class='main-header'>Factory Link <span style='font-size:1.5rem; color:#64748B;'>V1.4</span></div>", unsafe_allow_html=True)
        ...
        with st.container(border=True):
            ...
            with t2:
                ...
                with st.form("signup"):
                    ...
                    contact = st.text_input("연락처 (이메일 또는 전화번호 - 필수)")
                    ...
                    if st.form_submit_button("가입신청", use_container_width=True):
                        try:
                            ...
                            clean_comp = comp_name.strip() if comp_name else "개인회원" # 미입력 시 기본값

                            # [수정] 필수값 체크
                            if not clean_new_id or not clean_new_pw or not clean_contact:
                                st.error("🚨 아이디, 비밀번호, 연락처는 반드시 입력해야 합니다.")
                                ...
                                st.success("🎉 가입이 완료되었습니다! 로그인해주세요.")
                                st.balloons() # [기능] 가입 축하 풍선
                        except Exception as e:
                            st.error(f"오류 발생: {e}")
...
def main_app():
    with st.sidebar:
        try:
            ...
            # [기능 개선] 회원 정보 수정 (비밀번호 + 연락처 + 기업명)
            with st.expander("🔧 내 정보 수정"):
                with st.form("profile_update"):
                    st.caption("변경할 정보를 입력하세요 (비워두면 기존 유지)")
                    new_comp = st.text_input("기업명", value=current_user['company_name'])
                    new_contact = st.text_input("연락처", value=current_user['contact'])
                    ...
                    if st.form_submit_button("정보 수정", use_container_width=True):
                        ...
                        if new_pw.strip(): # 비밀번호 입력이 있을 때만 변경
    ...
    with tabs[1]:
        ...
        with st.form("reg_form"):
            ...
            desc = st.text_area("상세 내용 입력", height=150)
            ...
            if st.form_submit_button("등록 완료", type="primary", use_container_width=True):
                ...
                new_data = {
                    "id": str(int(time.time())), # ID를 문자열로 저장
    ...
    if st.session_state.get('is_admin'):
        with tabs[3]:
            ...
            # [기능 개선] 관리자가 회원 정보를 엑셀처럼 수정 가능
            edited_users = st.data_editor(
                users, 
                hide_index=True, 
                use_container_width=True,
                disabled=["user_id", "join_date"] # 아이디와 가입일은 수정 불가
            )
