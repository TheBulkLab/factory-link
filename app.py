...
def load_data(sheet_name):
    ...
    try:
        ...
        if df.empty or not all(col in df.columns for col in target_cols):
...
def login_page():
    ...
    with col1:
        ...
        st.markdown("### 대한민국 공단자원 거래 플랫폼")
        
        # [수정] 중개비 무료 삭제하고 장점 3가지로 변경
        st.markdown("""
        ##### 🚀 우리 공장에 필요한 모든 연결
...
def main_app():
    ...
    with tabs[0]:
        ...
        with st.container(border=True):
            ...
            with c_search:
                search_kw = st.text_input("🔍 통합 검색 (품목명, 기업명, 내용)", placeholder="예: 반응기, 폐산")
            with c_filter:
