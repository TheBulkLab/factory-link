...
# === [데이터] 산단 DB (간소화) ===
# 복잡한 산단 선택 대신 권역 중심 좌표 사용 (로딩 속도 개선)
REGION_DB = {
...
def render_footer():
    st.markdown("""
        <div class='footer-fixed'>
            💡 앱 문의 및 건의: <a href='https://cafe.naver.com/zjqlwkd' target='_blank'>화공재료연구회 카페</a>
        </div>
    """, unsafe_allow_html=True)
                ...
                with st.form("login_form"):
                    ...
                    if login_submitted:
                        try:
                            ...
                            # === [핵심 수정] 3. 관리자(admin/1234) 강제 복구 및 로그인 ===
                            elif clean_uid == "admin" and clean_pw == "1234":
                                admin_data = {
                                    "user_id": "admin", 
                                    "password_hash": hash_password("1234"), 
                                    "company_name": "관리자", 
                                    "contact": "admin@center.com", 
                                    "biz_no": "-", "is_verified": "TRUE", 
                                    "deal_count": 999, "reputation": 100.0, 
                                    "join_date": datetime.now().strftime("%Y-%m-%d")
                                }
                                
                                # 데이터가 있으면 덮어쓰고, 없으면 새로 생성
                                if not users.empty and 'user_id' in users.columns:
                                    if "admin" in users['user_id'].values:
                                        # 비번이 틀린 경우 -> 비번 리셋 효과
                                        users.loc[users['user_id'] == "admin", "password_hash"] = hash_password("1234")
...
def main_app():
    ...
    st.markdown("<div class='main-header'>🏭 Factory Link <span style='font-size:1.5rem; color:#64748B;'>V1.9.2</span></div>", unsafe_allow_html=True)
